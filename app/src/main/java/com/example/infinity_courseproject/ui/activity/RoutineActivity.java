...
import com.example.infinity_courseproject.ui.recyclerViewAdapter.RoutineRecViewAdapter;
...
public class RoutineActivity extends BaseActivity
    ...
    public void onItemClick(int position) {
        ...
        Intent intent = new Intent(this, RoutineAddEditActivity.class);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == ADD_ROUTINE_ACTIVITY_REQUEST_CODE && resultCode == RESULT_OK) {
            Integer startHour = data.getIntExtra(RoutineAddEditActivity.START_HOUR_REPLY, 24);
            Integer startMin = data.getIntExtra(RoutineAddEditActivity.START_MINUTE_REPLY, 0);
            if(startHour == 24) {
                startHour = null;
                startMin = null;
            }
            ArrayList<Event> events =
                    data.getParcelableArrayListExtra(RoutineAddEditActivity.EVENT_ARRAYLIST_REPLY);

            Routine routine = new Routine(
                    data.getStringExtra(RoutineAddEditActivity.TITLE_REPLY).trim(),
                    data.getBooleanArrayExtra(RoutineAddEditActivity.WEEKDAYS_REPLY),
